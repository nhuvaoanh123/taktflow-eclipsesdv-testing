"""Build and test verification for uProtocol Rust SDK (up-rust).

Core uProtocol types, UUri, UUID, transport traits in Rust.

Run on Linux laptop:
    pytest modules/uprotocol-rust/tests/build/test_build.py -v
"""
...
MODULE_DIR = Path(__file__).resolve().parents[4] / "uprotocol-up-rust"
CARGO_ENV = {
    **os.environ,
...
def module_dir():
    assert MODULE_DIR.exists(), f"up-rust not found at {MODULE_DIR}"
