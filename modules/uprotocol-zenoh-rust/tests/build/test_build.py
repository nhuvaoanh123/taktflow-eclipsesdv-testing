"""Build and test verification for uProtocol Zenoh Rust transport.

Run on Linux laptop:
    pytest modules/uprotocol-zenoh-rust/tests/build/test_build.py -v
"""
...
MODULE_DIR = Path(__file__).resolve().parents[4] / "uprotocol-up-transport-zenoh-rust"
CARGO_ENV = {
    **os.environ,
...
def module_dir():
    assert MODULE_DIR.exists(), f"zenoh-rust not found at {MODULE_DIR}"
    assert (MODULE_DIR / "Cargo.toml").exists()
    return MODULE_DIR
