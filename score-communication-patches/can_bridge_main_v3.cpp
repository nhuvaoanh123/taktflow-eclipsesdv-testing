...
static void decode(const struct can_frame& f, VehicleSignals& sig) {
    ...
    // ========== Real ECU messages (physical bench, taktflow_vehicle.dbc) ==========

    // 0x010 (16) CVC_Heartbeat: E2E(16) | ECU_ID:8 | OpMode:4 | FaultStatus:4
    case 0x010:
        if (f.can_dlc >= 4) {
            sig.vehicle_state = d[3] & 0x0F;
            sig.hsm_heartbeat = static_cast<uint32_t>((d[0] >> 4) & 0x0F);
        }
        break;

    // 0x011 (17) FZC_Heartbeat: E2E(16) | ECU_ID:8 | OpMode:4 | FaultStatus:4
    case 0x011:
        if (f.can_dlc >= 2) {
            sig.sequence = static_cast<uint32_t>((d[0] >> 4) & 0x0F);
        }
        break;

    // 0x200 (512) Steering_Status on real bench — but conflicts with SIL Motor_Command
    // Handled via the existing case 0x200 for SIL; on real bench, data layout differs
    // TODO: separate SIL vs real bench decode based on interface (vcan0 vs can0)

    // 0x210 (528) Steering_Command — from CVC to FZC
    case 0x210:
        if (f.can_dlc >= 4) {
            int16_t angle;
            memcpy(&angle, d + 2, 2);
            sig.steering_angle_deg = static_cast<float>(angle);
        }
        break;

    // 0x211 (529) Brake_Status: E2E(16) | BrakePos:8 | CmdEcho:8 | ServoCurrent:16 | Fault:4 | Mode:4
    case 0x211:
        if (f.can_dlc >= 4) {
            sig.pedal_position_pct = static_cast<float>(d[2]);
        }
        break;
